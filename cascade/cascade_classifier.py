...
class CascadeClassifier(object):
    ...
    def fit_transform(self, X_groups_train, y_train, X_groups_test, y_test, stop_by_test=True, VI=False):
        """
        ...
        VI=False
        if VI:
            all_feature_importance = np.zeros((1, n_estimators * n_classes + X_train.shape[1]), dtype=np.float64)
        else:
            all_feature_importance = None
        ...
        try:
            ...
            while 1:
                ...
                if layer_id==0:
                    ...
                    RF.fit(X_cur_train, y_train)
                    LOGGER.info("[RF test Accuracy={}]".format(RF.score(X_cur_test,y_test)))
                ...
                y_test_proba_li_list.append(y_test_proba_li)
                ...
                if opt_layer_id == layer_id:
                    opt_datas = [opt_layer_id, X_cur_train, y_train, X_cur_test, y_test, train_aa_list[opt_layer_id],
                                 test_aa_list[opt_layer_id], train_acc_list[opt_layer_id], test_acc_list[opt_layer_id],
                                 kappa_list[opt_layer_id], all_feature_importance]
                # early stop
                if self.early_stopping_rounds > 0 and layer_id - opt_layer_id >= self.early_stopping_rounds:
                    ...
                    LOGGER.info("[Result][test AA]:" + str(test_aa_list[opt_layer_id]))
                    ...
                    return opt_datas,y_test_proba_li_list[opt_datas[0]]
                ...
                # inc layer_id
                layer_id += 1
            opt_datas = [self.max_layers-1, X_cur_train, y_train, X_cur_test, y_test, train_aa_list[-1], test_aa_list[-1], train_acc_list[-1], test_acc_list[-1], kappa_list[-1], all_feature_importance]
            ...
            LOGGER.info("[Result][Reach Max Layer] max_layer_num={}, accuracy_train={:.2f}%, accuracy_test={:.2f}%, AA accuracy_train={:.2f}%, AA_accuracy_test={:.2f}%".format(
                    self.max_layers-1, train_acc_list[-1], test_acc_list[-1], np.mean(train_aa_list[-1]),
