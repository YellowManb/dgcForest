...
LOGGER = get_logger("gcforest_HSI_rw.estimators.kfold_wrapper")
...
class KFoldWrapper(object):
    ...
    def fit_transform(self, X, y, y_stratify, cache_dir=None, test_sets=None, eval_metrics=None, keep_model_in_mem=True,
        ...
        for k in range(self.n_folds):
            ...
            # fit on k-fold train
            temp_X = X[train_idx].reshape((-1, n_dims))
            temp_y = y[train_idx].reshape(-1)
            est.fit(X[train_idx].reshape((-1, n_dims)), y[train_idx].reshape(-1), cache_dir=cache_dir)
            importance = est.est.feature_importances_
            ...
            if k == 0:
                if len(X.shape) == 2:
                    if VI:
                        import copy
                        feature_importances = copy.deepcopy(est.est.feature_importances_)
            elif VI:
                feature_importances += est.est.feature_importances_
        ...
        if VI:
            return y_probas, feature_importances
        else:
            return y_probas
