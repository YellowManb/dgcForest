...
LOGGER = get_logger("gcforest_HSI_rw.gcnet")
...
class FGNet(object):
    ...
    def __init__(self, net_config, data_cache):
        ...
        for layer_config in net_config["layers"]:
            layer, layer_type = get_layer(layer_config, self.data_cache)
            layer.model_disk_base = model_disk_base
            self.layers.append(layer)
            self.name2layer[layer.name] = layer
            self.layers_type.append(layer_type)
    ...
    def fit_transform(self, X_train, y_train, X_test, y_test, train_config):
        ...
        for li, layer in enumerate(self.layers):
            # layer_type = self.layers_type[li]
    ...
    def save_outputs(self, phase, save_y=True, save_path=None, epoch=None):
...
class FGTrainConfig(object):
    def __init__(self, train_config, epoch=None, data_set=None):
        ...
        self.data_cache = DataCache(train_config["data_cache"], epoch=epoch)
        self.epoch = epoch
